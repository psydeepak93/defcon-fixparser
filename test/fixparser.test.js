...
describe('FIXParser', () => {
    ...
    describe('#parse: SDBK Price Correction of Previous Execution', () => {
        ...
        it('should have ExecID value NF 0039/03022010 001001002', () => {
            ...
            expect(messages[0].data[index].value).toBe(
                'NF 0039/03022010 001001002',
            );
            index++;
        });
    ...
    describe('#parse: ExecutionReport', () => {
        let messages = fixParser.parse(
                '8=FIX.4.4\x019=1753\x0135=8\x01523=S|E|B Auto-pricing\x01',
            ),
            index = 0;
    ...
    describe('#getBriefDescription', () => {
        describe('should generate a brief description', () => {
            ...
            it('New Order Single', () => {
                ...
                expect(messages[0].getBriefDescription()).toBe(
                    'BUY 100 CVS MKT DAY',
                );
            });
            ...
            it('BBSS Max Quantity Order (99,999,900 shares)', () => {
                ...
                expect(messages[0].getBriefDescription()).toBe(
                    'BUY 99999900 MMM LMT @50.00 DAY',
                );
            });
            ...
            it('Multiple messages', () => {
                ...
                expect(messages[4].getBriefDescription()).toBe(
                    'BUY 10000 MSFT MKT DAY',
                );
                expect(messages[5].getBriefDescription()).toBe(
                    '10000 @0.00 LvsQty 0',
                );
                expect(messages[6].getBriefDescription()).toBe(
                    '10000 @12.30 LvsQty 0',
                );
                expect(messages[7].getBriefDescription()).toBe(
                    'BUY 10000 ORCL MKT DAY',
                );
                expect(messages[8].getBriefDescription()).toBe(
                    '10000 @0.00 LvsQty 0',
                );
                expect(messages[9].getBriefDescription()).toBe(
                    '10000 @12.30 LvsQty 0',
                );
                expect(messages[10].getBriefDescription()).toBe(
                    'BUY 10000 SPY LMT @10.00 DAY',
                );
                expect(messages[11].getBriefDescription()).toBe(
                    '10000 @0.00 LvsQty 0',
                );
                expect(messages[12].getBriefDescription()).toBe(
                    'BUY 10000 SPY',
                );
                expect(messages[13].getBriefDescription()).toBe('Reject');
                expect(messages[14].getBriefDescription()).toBe(
                    'BUY 10000 SPY',
                );
    ...
    function processTest(fixMessage) {
        describe(`#parse: ${fixMessage.description}`, () => {
            ...
            it('should have parsed the FIX message', () => {
                expect(messages.length).toEqual(
                    fixMessage.numMessages ? fixMessage.numMessages : 1,
                );
            ...
            it(`should have valid BodyLength: ${messages[0].bodyLengthValid}`, () => {
                expect(messages[0].bodyLengthValid).toBe(
                    fixMessage.bodyLengthValid,
                    `[actual: ${messages[0].bodyLengthValue}, expected: ${messages[0].bodyLengthExpected}]`,
                );
            });

            it(`should have valid CheckSum: ${messages[0].checksumValid}`, () => {
                expect(messages[0].checksumValid).toBe(
                    fixMessage.checksumValid,
                    `[actual: ${messages[0].checksumValue}, expected: ${messages[0].checksumExpected}]`,
                );
            });
        });
    }
    ...
    describe('#parse: checksum validation', () => {
        const messages = fixParser.parse(
